...
#dejong5_configs_df.to_csv("clonal-selection-algorithm/dejong_configs_clonalg.csv", sep=";", index=False)
...
#lang_configs_df.to_csv("clonal-selection-algorithm/lang_configs_clonalg.csv", sep=";", index=False)
...
selection_rate   = configs["config4"][0]
clone_rate       = configs["config4"][1]
mutation_range   = configs["config4"][2]
population_size  = configs["config4"][3]
number_of_epochs = configs["config4"][4]
x_size = 2
x_range = [-65.536, 65.536]


bestFitnessPerEpochDejong = []

population = [[random.uniform(x_range[0], x_range[1]) for j in range(x_size+1)] for i in range(population_size)]
population.sort(key=sort_func)

#bestFitnessPerEpochDejong.append(population[0][-1])
...
df1.to_csv("clonal-selection-algorithm/csa_dejong_best.csv", sep=";", index=None)
